...
DEFAULT_QOS = 1 # by default - prof said not to mess with this too much for know
DEFAULT_PORT =  1883 # 8883 # this port uses TLS/SSL - no eavesdropping on traffic

MQTT_BROKER_ADDRESS="172.17.0.1" # this will be temporarily a tailscale IP
...
# ----------------- API >> Wrapper (dis)connect + send functions
    ...
    def send_payload(self, packet: Dict[str,Any]) -> str:
        if self.connection_state != ConnectionState.CONNECTED:
            raise RuntimeError("Client Disconnected from Broker...")

        # build the payload and assign an id
        payload = self.build_payload(packet)
        ...
        if result.rc != mqtt.MQTT_ERR_SUCCESS:
            self._update_message_status(message_id, MessageStatus.FAILED)
...
# ---------- Connection/Disconnection from Broker -------------
    ...
    def _on_connect(self, client, userdata, flags, rc, properties=None) -> None:
        if rc == 0:
            ...
            self._mqtt_client.subscribe(self.feedback_topic, qos=self.qos)
    ...
    def _on_message(self, client, userdata, msg) -> None:
        ...
        if not message_id or not status_str:
            self._last_error = "Incomplete payload on feedback topic"
            return
        ...
        status = status_map.get(str(status_str).lower())
        if status is None:
